    ...
    var views = (function(){
        ...
        views.loadvotes = function(page){
            async.waterfall([
                ...
                function(re, done){
                    $("#votes").append(templates.votes(re.votes, re.pagesize * (page || 0)))
                    done(null, re)
                },
                function(re, done){
        ...
        views.paginate = function(page, pagesize, total){
            page = page || 0
            if (page > 0){
                $("#pagin #first").attr("href", "/r?p=0")
                $("#pagin #prev").attr("href", "/r?p=" + (page - 1))
            } else {
            ...
            if (page < Math.ceil(total / pagesize - 1)){
                $("#pagin #next").attr("href", "/r?p=" + (parseInt(page) + 1))
                $("#pagin #last").attr("href", "/r?p=" + Math.ceil(total / pagesize - 1))
            } else {
