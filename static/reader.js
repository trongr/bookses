var bok = function(x){
    ...
    var api = (function(){
        ...
        api.lockdrawing = function(id, done){
            $.ajax({
                url: "/comment/" + id + "/lock",
                type: "post",
                success: function(re){
    ...
    var templates = (function(){
        ...
        templates.book_info = function(book){
            ...
            var html = "<div class='boks_book_info'>"
                ...
                +          "<div class='boks_spinner'><i class='icon-spin icon-globe'></i></div>"
                +          "<div class='boks_latest_comments_header'>"
                +              "<span id='latestactivitylabel'>Latest Activity.</span>"
        ...
        templates.reader = function(text){
            var html = "<div id='" + o.bID + "' class='boks_reader'>"
                ...
                +                  "<div id='click_to_reveal'><span>QUICK START GUIDE</span><br>Click on the paragraphs to see and write comments</div>"
        ...
        templates.p_menu = function(p){
            ...
            var html = "<div class='boks_p_menu data' data-p='" + p + "'>"
                ...
                +              "<button class='boks_reply'>Add an illustration</button>"
                +          "</div>"
        ...
        templates.reply_box = function(p, parentid){
            ...
            var html = "<div class='boks_reply_box data' " + datap + " " + dataparent + ">"
                ...
                +          "<div class='boks_reply_text'><textarea placeholder='Add a comment'></textarea></div>"
                ...
                // +              "</div>"
                // +          "</div>"
                +          "<input class='boks_reply_artists' placeholder='Artist Names'>"
        ...
        templates.comment = function(comment){
            ...
            //         +       "<button class='lockdrawing " + (comment.locked ? "red" : "green") + "'>"
            //         +            (comment.locked ? "<i class='icon-lock'></i> " + comment.locker : "<i class='icon-unlock'></i>")
            //         +       "</button>"
            //         +       "<span>Don't forget to lock this drawing before you start working on it. The lock will expire in 10 minutes.</span>"
            //         +  "</div>"
            // }
            var artists = ""
            if (comment.artists){
                artists += "<div class='boks_comment_artists'>" + comment.artists + "</div>"
            }
            var approved = ""
        ...
        templates.short_comment = function(comment){
            ...
            // var lock = ""
            // if (comment.img){
            //     lock = "<div class='lockboxshortcomment'>"
            //         +       (comment.locked ? "<span>" + comment.locker + "</span>" : "")
            //         +       "<button class='lockdrawing " + (comment.locked ? "red" : "green") + "'>"
            //         +            (comment.locked ? "<i class='icon-lock'></i>" : "<i class='icon-unlock'></i>")
            //         +       "</button>"
            //         +  "</div>"
            // }
            var artists = ""
            ...
            var html = "<div class='short_comment_box " + is_edit + "'>"
                ...
                +                  "<div class='comment_created'>" + Date.create(comment.created).long() + "</div>"
                +                  "<div class='comment_modified'>" + Date.create(comment.modified).long() + "</div>"
                // +                  lock
                +              "</div>"
                +               img
                +               youtube
            ...
            async.waterfall([
                ...
                function(text, done){
                    ...
                    dom.content_right = dom.box.find(".boks_content_right")
                        ...
                        .on("click", ".lockdrawing", bindings.lockdrawing)
                    done(null)
                },
                function(done){
        ...
        views.load_latest_comments = function(page, done){
            async.waterfall([
                ...
                function(comments, done){
                    ...
                    var count = 0
                    for (var i=0; i < comments.length; i++){
                        if (new Date(new Date().getTime() - fivemins) < new Date(comments[i].created))
                            count++
                    }
                    $("#latestcount").html(count + " new in last 5 min")
                    done(null)
                }
            ], function(er){
                done(er)
    ...
    var bindings = (function(){
        ...
        bindings.click_reply = function(){
            ...
            box.html(templates.reply_box(p, id)).find("textarea").focus()
        ...
        bindings.click_reply_post = function(){
            ...
            var comment = data_box.find(".boks_reply_text textarea").val().trim()
            var tags = u.get_tags(comment)
            var artists = data_box.find(".boks_reply_artists").val().trim()
            ...
            } else if (!img && !comment) return msg.error("empty")
            ...
            if (artists) data.append("artists", artists)
            ...
            var processing = data_box.find(".boks_reply_processing").show()
        ...
        bindings.lockdrawing = function(){
            var id = $(this).closest(".data").attr("data-id")
            $(this).html("<i class='icon-lock'></i>").removeClass("green").addClass("red")
            async.waterfall([
                function(done){
                    api.get_comment(id, function(er, note){
                        done(er, note)
                    })
                },
                function(note, done){
                    if (note.locked){
                        msg.error("Hey! " + note.locker + " is working on this version. To avoid conflict and maximize effort please come back in the next round to improve the updated illustration.", 10000)
                        done({goto:1})
                    } else {
                        api.lockdrawing(id, function(er){
                            done(er)
                        })
                    }
                },
            ], function(er){
                if (er && !er.goto) console.log(JSON.stringify(er, 0, 2))
            })
        }
